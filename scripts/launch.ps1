...
Write-Host "  ║   A X O N  — Intelligence   ║" -ForegroundColor Cyan
Write-Host "  ╚══════════════════════════════╝" -ForegroundColor Cyan
Write-Host ""
...
function Is-Installed($p){$r=& $venvPip show $p 2>$null;return($null -ne $r -and $r -ne "")}
...
}else{Write-Host "  [SKIP] PyTorch nightly ok ($tv)" -ForegroundColor DarkGray}
...
Write-Host "  [4/5] mkdir data..." -ForegroundColor Yellow
